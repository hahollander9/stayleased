...
PR #7 landed that fix (recorded here as DECISION 53) while this build was in flight, and main is merged in here, so
this branch now carries it — the two tests pass. This build's decisions are numbered 54–59; see the recovery note below for why. Recorded because the advice that used to stand in `CLAUDE.md` — "a red there =
...
**Recovered here: two BUILDLOG entries and two DECISIONS that a web upload removed from main.**
Commit `6fef829` ("Add files via upload") landed the property-scoped-settings work by replacing whole
files from a local copy taken before #7 merged. Code was unaffected — `voidApPayment`'s fix is still
on main — but the *record* was not. Both of #7's BUILDLOG entries were gone, and its DECISIONS 50 and
51 had been overwritten by two unrelated entries reusing the same numbers, so main carried a money-bug
fix with no written account of why it exists. This branch still held them from its own merge of #7,
which is the only reason they are recoverable; they are restored above and as DECISIONS 52–53, and this
build's own entries shift to 54–59 because the upload's 50–51 are already published on main.

This is the same hazard `CLAUDE.md` already records against `.github/workflows/ci.yml` ("lost to a
web-UI upload"), and it is worse than the note suggests: a dot-directory vanishing is at least visible,
whereas an append-only log silently reverting to an older revision looks exactly like a file that was
never written to. `.github/` survived this time. The engineering memory did not.
