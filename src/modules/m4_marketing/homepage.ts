...
const MARKETING_CSS = `
:root { --ink:#0b1220; --ink2:#3c4657; --mut:#66707f; --blue:#2563eb; --blue-d:#1d4ed8; --line:#e5e9f0; --bg:#ffffff; --bg2:#f6f8fc; }
* { box-sizing: border-box; margin: 0; }
body.mk { font: 16px/1.6 -apple-system, "Segoe UI", Roboto, Inter, sans-serif; color: var(--ink); background: var(--bg); -webkit-font-smoothing: antialiased; scroll-behavior: smooth; }
.mk-wrap { max-width: 1160px; margin: 0 auto; padding: 0 22px; }
a { color: inherit; text-decoration: none; }

/* nav */
.mk-nav { position: sticky; top: 0; z-index: 60; background: rgba(255,255,255,.92); backdrop-filter: blur(10px); border-bottom: 1px solid var(--line); }
.mk-nav-in { display: flex; align-items: center; gap: 26px; height: 64px; }
.mk-logo { display: flex; align-items: center; gap: 8px; font-size: 19px; font-weight: 500; }
.mk-logo b { color: var(--blue); font-weight: 800; }
.mk-menu { display: flex; align-items: center; gap: 4px; flex: 1; }
.mk-item { position: relative; }
.mk-item-btn { display: flex; align-items: center; gap: 5px; background: none; border: 0; font: inherit; font-weight: 600; font-size: 14.5px; color: var(--ink2); padding: 9px 12px; border-radius: 8px; cursor: pointer; }
.mk-item-link { font-weight: 600; font-size: 14.5px; color: var(--ink2); padding: 9px 12px; border-radius: 8px; }
.mk-item-btn:hover, .mk-item-link:hover { background: var(--bg2); color: var(--ink); }
.mk-drop { position: absolute; left: 0; top: calc(100% + 6px); background: #fff; border: 1px solid var(--line); border-radius: 14px; box-shadow: 0 18px 50px rgba(16,24,40,.14); padding: 10px; display: none; }
.mk-item.open .mk-drop { display: block; }
@media (hover: hover) { .mk-item:hover .mk-drop { display: block; } }
.mk-drop-grid { display: grid; grid-template-columns: repeat(2, 280px); gap: 2px; }
.mk-drop-grid a { display: flex; flex-direction: column; gap: 1px; padding: 9px 11px; border-radius: 9px; }
.mk-drop-grid a:hover { background: var(--bg2); }
...
/* buttons */
.mk-btn { display: inline-flex; align-items: center; justify-content: center; gap: 7px; font-weight: 700; font-size: 14.5px; border-radius: 10px; padding: 10px 17px; border: 0; cursor: pointer; transition: transform .12s ease, box-shadow .12s ease, background .12s ease; }
...
.mk-btn-solid:hover { background: var(--blue-d); transform: translateY(-1px); }
.mk-btn-line { border: 1.6px solid #c9d4ea; color: var(--ink); background: #fff; }
.mk-btn-line:hover { border-color: var(--blue); color: var(--blue); }
.mk-btn-ghost { color: var(--ink2); background: transparent; }
.mk-btn-ghost:hover { color: var(--ink); background: var(--bg2); }

/* hero */
.mk-hero { background: radial-gradient(1200px 500px at 75% -10%, rgba(37,99,235,.16), transparent 60%), linear-gradient(180deg, #f8faff, #fff 75%); border-bottom: 1px solid var(--line); }
.mk-hero-in { display: grid; grid-template-columns: 1.05fr .95fr; gap: 44px; align-items: center; padding: 72px 22px 78px; }
.mk-kicker { display: inline-block; font-size: 12px; font-weight: 800; letter-spacing: 1.3px; text-transform: uppercase; color: var(--blue); background: rgba(37,99,235,.09); border: 1px solid rgba(37,99,235,.22); padding: 4px 11px; border-radius: 99px; margin-bottom: 18px; }
.mk-hero h1 { font-size: clamp(34px, 4.6vw, 54px); line-height: 1.06; letter-spacing: -1.2px; font-weight: 800; }
...
/* hero product mock */
.mk-frame { background: #fff; border: 1px solid var(--line); border-radius: 16px; box-shadow: 0 30px 70px rgba(16,24,40,.16); overflow: hidden; }
...
.mk-frame-kpis div { border: 1px solid var(--line); border-radius: 10px; padding: 10px 12px; }
.mk-frame-kpis b { display: block; font-size: 18px; letter-spacing: -.4px; }
...
.mk-frame-chart i { flex: 1; background: linear-gradient(180deg, #6d8df3, #2563eb); border-radius: 4px 4px 2px 2px; min-height: 12%; }
...
.mk-plat { border: 1px solid var(--line); background: #fff; border-radius: 16px; padding: 28px; transition: box-shadow .15s ease, transform .15s ease; }
.mk-plat:hover { box-shadow: 0 18px 44px rgba(16,24,40,.12); transform: translateY(-2px); }
...
.mk-more { display: inline-block; margin-top: 14px; font-weight: 700; color: var(--blue); font-size: 14px; }

/* ontology stack */
.mk-stack { display: grid; gap: 8px; max-width: 860px; }
.mk-layer { border: 1px solid var(--line); background: #fff; border-radius: 13px; overflow: hidden; }
...
.mk-lnum { flex: none; width: 34px; height: 34px; border-radius: 10px; background: rgba(37,99,235,.1); color: var(--blue); font-weight: 800; display: flex; align-items: center; justify-content: center; }
...
.mk-plus::before, .mk-plus::after { content: ''; position: absolute; background: var(--mut); border-radius: 2px; transition: transform .16s ease; }
...
.mk-layer[open] { border-color: rgba(37,99,235,.5); box-shadow: 0 8px 26px rgba(37,99,235,.09); }
.mk-lbody { padding: 0 18px 17px 66px; color: var(--ink2); font-size: 15px; }
...
.mk-level { position: relative; display: flex; gap: 18px; background: #fff; border: 1px solid var(--line); border-radius: 13px; padding: 17px 20px; transition: transform .14s ease, box-shadow .14s ease; }
.mk-level:hover { transform: translateX(4px); box-shadow: 0 10px 30px rgba(16,24,40,.1); }
.mk-level-cube { flex: none; width: 30px; }
...
.mk-card { border: 1px solid var(--line); background: #fff; border-radius: 14px; padding: 22px; transition: transform .14s ease, box-shadow .14s ease; }
.mk-card:hover { transform: translateY(-3px); box-shadow: 0 14px 36px rgba(16,24,40,.1); }
...
.mk-phone { width: 250px; border: 1px solid var(--line); border-radius: 22px; background: #fff; box-shadow: 0 20px 46px rgba(16,24,40,.12); padding: 20px 18px 22px; }
.mk-phone-mid { transform: translateY(-14px); }
...
.mk-ph-btn { margin-top: 12px; background: var(--blue); color: #fff; text-align: center; font-weight: 700; border-radius: 10px; padding: 10px; }

/* governance */
.mk-dark { background: linear-gradient(180deg, #0b1220, #101b33); color: #e7edf7; }
...
.mk-checks li { padding-left: 28px; position: relative; font-size: 15px; color: #d6deec; }
.mk-checks li::before { content: '✓'; position: absolute; left: 0; top: 0; width: 19px; height: 19px; border-radius: 99px; background: rgba(59,130,246,.25); color: #93c5fd; font-size: 12px; font-weight: 800; display: flex; align-items: center; justify-content: center; }
...
.mk-form-card { background: #fff; border: 1px solid var(--line); border-radius: 16px; padding: 24px; box-shadow: 0 16px 40px rgba(16,24,40,.09); }
...
.mk-form-card input, .mk-form-card select { font: inherit; font-weight: 400; padding: 9px 11px; border: 1.4px solid #d6dce8; border-radius: 9px; background: #fff; }
.mk-form-card input:focus, .mk-form-card select:focus { outline: none; border-color: var(--blue); }
.mk-form-full { margin-bottom: 14px; }
.mk-thanks { font-size: 15.5px; color: var(--ink2); }

/* footer */
.mk-foot { background: #0b1220; color: #aeb9cd; padding: 52px 0 26px; }
...
.mk-foot-grid a { display: block; font-size: 13px; padding: 3.5px 0; color: #aeb9cd; }
.mk-foot-grid a:hover { color: #fff; }
...
/* reveal */
.mk-reveal { opacity: 0; transform: translateY(14px); transition: opacity .5s ease, transform .5s ease; }
